...
namespace BrickBuilder.Utilities
{
    ...
    public sealed class FastObjImporter
    {
            ...
            for (int i = 0; i < text.Length; i++)
            {
                if (text[i] == '\n')
                {
                    ...
                    else if (sb[0] == 'f' && sb[1] == ' ')
                    {
                        ...
                        j = 1;
                        while (
                            j + 2 < info) //Create triangles out of the face data.  There will generally be more than 1 triangle per face.
                        {
        ...
        private float ParseFloat(StringBuilder value)
        {
            ...
            return result;
            */
            return Convert.ToSingle(value.ToString(), CultureInfo.InvariantCulture); // unoptimized code go brrr
