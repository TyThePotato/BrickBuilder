...
namespace BrickBuilder.World
{
    ...
    public class MapEditor : MonoBehaviour
    {
        ...
        private static void TrySelectArea(Vector2 screenStart, Vector2 screenEnd)
        {
            ...
            // generate selection chunks finally
        ...
        public static void SelectBrick(Brick brick, bool addToSelection = false, bool fromUI = false, bool generateChunks = true)
        {
            ...
            if (generateChunks) UpdateSelectedChunks();
        ...
        public static void DeselectBrick(Brick brick, bool fromUI = false)
        {
            ...
            // regenerate chunks of any selected bricks so that their mesh contains the brick
            RegenerateChunksOfSelectedBricks();
            
            // remove brick from selection
            SelectedBricks.Remove(brick);
            
            // update selection chunks to only contain selected bricks & remove selected bricks from main chunks
            UpdateSelectedChunks();
        ...
        public static void DeselectAllBricks(bool fromUI = false)
        {
            ...
            // regenerate chunks of all selected bricks
            RegenerateChunksOfSelectedBricks();
            
            // remove all selected bricks from selection
            SelectedBricks.Clear();
            
            // update selection chunks to not have any bricks
            UpdateSelectedChunks();
        ...
        private static void UpdateSelectedChunks() {
            List<Chunk> emptyChunks = new List<Chunk>(); // store empty chunks for later deletion
            
            // check if any selected chunks contain no-longer selected bricks
            for (int i = 0; i < SelectedChunks.Count; i++)
            {
                List<Guid> selectedBricks = new List<Guid>();
                bool regenChunk = false;

                // only check chunk if there are even selected bricks
                if (selectedBricks.Count > 0)
                {
                    for (int j = 0; j < SelectedChunks[i].Bricks.Count; j++)
                    {
                        for (int k = 0; k < SelectedBricks.Count; k++)
                        {
                            if (SelectedBricks[k].ID == SelectedChunks[i].Bricks[j])
                            {
                                // this brick is still selected
                                selectedBricks.Add(SelectedBricks[k].ID);
                            }
                            else
                            {
                                // this brick is not selected, meaning the chunk contains an unselected brick
                                regenChunk = true;
                            }
                        }
                    }
                ...
                {
                    // check to see if this chunk contains bricks
                    // if it does, then it needs to be regenerated
                    if (SelectedChunks[i].Bricks.Count > 0)
                    {
                        regenChunk = true;
                    }
                }
                ...
                if (regenChunk)
                {
                    SelectedChunks[i].Bricks = selectedBricks;
                    MapBuilder.RegenerateChunk(SelectedChunks[i], SelectedBricks);

                    // check if chunk is empty
                    // done after regeneration as empty chunks get auto deleted there
                    if (selectedBricks.Count == 0) {
                        emptyChunks.Add(SelectedChunks[i]);
                    }
                }
            }

            SelectedChunks.AddRange(MapBuilder.SeparateBricks(SelectedBricks, SelectedChunks));

            for (int i = 0; i < SelectedChunks.Count; i++)
            {
                Material[] chunkMaterial = new[]
                {
                    MaterialCache.GetMaterial(MaterialCache.FaceType.Smooth, Vector2.one, SelectedChunks[i].Alpha,
                        true),
                    MaterialCache.GetMaterial(MaterialCache.FaceType.Stud, Vector2.one, SelectedChunks[i].Alpha, true),
                    MaterialCache.GetMaterial(MaterialCache.FaceType.Inlet, Vector2.one, SelectedChunks[i].Alpha, true),
                    MaterialCache.GetMaterial(MaterialCache.FaceType.Spawnpoint, Vector2.one, SelectedChunks[i].Alpha,
                        true)
                };
                SelectedChunks[i].GameObject.GetComponent<MeshRenderer>().materials = chunkMaterial;
            }
        }

        private static void RegenerateChunksOfSelectedBricks()
        {
            for (int i = 0; i < MapBuilder.Chunks.Count; i++)
            {
                bool regenChunk = false;
                for (int j = 0; j < SelectedBricks.Count; j++)
                {
                    if (MapBuilder.Chunks[i].Bricks.Contains(SelectedBricks[j].ID))
                    {
                        regenChunk = true;
                    }
                }
                ...
                if (regenChunk)
                    MapBuilder.RegenerateChunk(MapBuilder.Chunks[i], EditorMain.OpenedMap.Bricks);
            }
        }

        private static void DeleteSelectionChunks()
        {
            for (int i = 0; i < SelectedChunks.Count; i++)
            {
                Destroy(SelectedChunks[i].GameObject);
                SelectedChunks[i].Mesh.Clear();
            }
        ...
        public static void ImportBricks(List<Brick> bricks, bool newID = true)
        {
            ...
            {
                if (newID)
                    bricks[i].ID = Guid.NewGuid();
                
                EditorMain.OpenedMap.Bricks.Add(bricks[i]);
                EditorUI.CreateHierarchyElement(bricks[i],false);
            }
            
            MapBuilder.Chunks.AddRange(MapBuilder.MeshBricks(bricks, MapBuilder.Chunks, true, false));
        }

        public static void RemoveBrick(Brick brick, bool registerCommand = true, bool deleteSelectionChunks = true)
        {
            if (registerCommand)
            {
            ...
            else
            {
                ...
                MapBuilder.RemoveBrick(brick);

                // delete hierarchy element
                EditorUI.RemoveHierarchyElement(brick.ID);

                // Delete Brick Object
                EditorMain.OpenedMap.Bricks.Remove(brick);
            }
            
            if (deleteSelectionChunks)
            {
                DeleteSelectionChunks();
            }
        }

        public static void RemoveBricks(List<Brick> bricks, bool registerCommand = true, bool deleteSelectionChunks = true)
        {
            if (registerCommand)
            {
            ...
            else
            {
                ...
                MapBuilder.RemoveBricks(bricks.ToArray());

                for (int i = 0; i < bricks.Count; i++)
                {
                    EditorUI.RemoveHierarchyElement(bricks[i].ID);
                    EditorMain.OpenedMap.Bricks.Remove(bricks[i]);
                }
            }
